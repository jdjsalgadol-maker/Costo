...
def ejecutar_pipeline_etl(file_real, file_insumos, file_vacunas):
    ...
    # Filtro de limpieza de base de datos
...
with st.expander("📁 1. Cargar Bases de Datos (ETL)", expanded=not 'df_procesado' in st.session_state):
    ...
    if file_real and file_insumos and file_vacunas:
        if st.button("Procesar Modelo de Datos", type="primary"):
            with st.spinner('Construyendo modelo relacional...'):
...
if 'df_procesado' in st.session_state:
    ...
    with col_chart2:
        st.markdown("#### Top 10 Mayores Desviaciones")
        # Filtrar solo los que tienen sobrecosto
    ...
    st.dataframe(df_display.style.apply(estilo_filas, axis=1).format({'Costo Real': '${:,.0f}'}), use_container_width=True, height=250)
